...
function highlightMetrics(container) {
  const regex = /(±|≥|≤)?\s*(\d+(?:[.,]\d+)?(?:½)?)(\s*(–|-|—|to)\s*(\d+(?:[.,]\d+)?(?:½)?))?(\s*[/]?\s*)?(%|ms|s|sec|seconds?|min|minutes?|hr|hours?|°C|°F|K|nm|mm|cm|m|µm|um|angstrom|Å|kg|g|mg|µg|ug|mL|ml|L|Hz|kHz|MHz|GHz|Ω|Ohm|V|mV|A|mA|W|kW|N|Nm|psi|in|ft|°)\b/gi;
